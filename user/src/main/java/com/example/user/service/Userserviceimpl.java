...
import com.example.user.mapper.User;
...
public class Userserviceimpl implements Userservice {
    ...
    public Map FindCallery(int userid,int page) {
        ...
        for(int i=0;i<callery.size();i++) {
            ...
            m.put("num",num);
            System.out.println(callery.get(i).get("id") + "" + callery.get(i).get("title") + "" + callery.get(i).get("time"));
            //根据图事ID获取图事相对应图片及文字
            List<Map> calleryinfo=u.FindCalleryinfo((int)(callery.get(i).get("id")));
            m.put("info",calleryinfo);
            for (int j=0;j<calleryinfo.size();j++){
                System.out.println("图片"+calleryinfo.get(j).get("id")+":"+calleryinfo.get(j).get("image")+"故事："+calleryinfo.get(j).get("content"));
            }
            //创建评论List
            ...
            for(int j=0;j<comment.size();j++){
                ...
                System.out.println(comment.get(j).get("nickname") + "评论了：" +comment.get(j).get("content")+"时间："+comment.get(j).get("time"));
                //创建评论回复的List
                ...
                for (int x=0;x<commentback.size();x++){
                    ...
                    System.out.println(commentback.get(x).get("nickname") + "回复了 " +touserid+"："+commentback.get(x).get("content"));
                }
